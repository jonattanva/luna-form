...
import { getInitialCount } from '@/packages/luna-core/src/util/list'
import type { List } from '@/packages/luna-core/src/type'

test.describe('List', { tag: ['@unit'] }, () => {
  test('should return 0 when no min and no value', () => {
    const list: List = {
      name: 'items',
      type: 'list',
      fields: [],
  ...
  test('should return 0 if value is not an array and no min', () => {
    const list: List = {
      name: 'items',
      type: 'list',
      fields: [],
    ...
    expect(getInitialCount(list, value)).toBe(0)
  })
  ...
  test('should return 0 when value is null', () => {
    ...
    expect(getInitialCount(list, null)).toBe(0)
  })
})
